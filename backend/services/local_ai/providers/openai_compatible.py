"""Generic OpenAI-compatible local provider (Jan, LlamaFile, Llamacpp server, etc.)."""
...
logger = logging.getLogger("synaptiq.local_ai.providers.openai_compatible")


class OpenAICompatibleProvider(LocalAIProvider):
    """Provider for any server that speaks the OpenAI Chat Completions API."""

    def __init__(self, base_url: str, api_key: str = "local", name: str = "openai_compatible") -> None:
    ...
    def _api_base(self) -> str:
        url = self._base_url
        if not (url.endswith("/v1") or "/v1/" in url):
            url = url + "/v1"
        return url

    def _client(self, timeout: float = 30.0) -> httpx.AsyncClient:
        headers = {"Authorization": f"Bearer {self._api_key}"} if self._api_key else {}
        return httpx.AsyncClient(base_url=self._api_base(), timeout=timeout, headers=headers)
    ...
    async def list_models(self) -> list[LocalModelInfo]:
        try:
            ...
            logger.debug("openai_compatible list_models error: %s", exc)
            return []
        ...
        for m in data.get("data", []):
            ...
            models.append(LocalModelInfo(
                ...
                context_window=int(m.get("context_window") or m.get("max_context_length") or 0),
                available=True,
            ))
        return models
    ...
    async def chat(
        ...
        logger.debug(
            "%s.chat model=%s input=%d output=%d latency_ms=%d",
            self._name, model, input_tokens, output_tokens,
