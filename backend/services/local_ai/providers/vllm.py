...
logger = logging.getLogger("synaptiq.local_ai.providers.vllm")


class VLLMProvider(LocalAIProvider):
    ...
    async def list_models(self) -> list[LocalModelInfo]:
        try:
            ...
            logger.debug("vllm list_models error: %s", exc)
            return []
        ...
        for m in data.get("data", []):
            ...
            models.append(LocalModelInfo(
                ...
                context_window=int(m.get("max_model_len", 0)),
                available=True,
            ))
        return models
    ...
    async def chat(
        ...
        logger.debug(
            "vllm.chat model=%s input=%d output=%d latency_ms=%d",
