...
import { User, Calendar, Coins, TrendingUp, ArrowUpRight, Sparkles, X } from "lucide-react";
...
function RecRow({ type, icon: Icon, iconBg, iconColor, title, desc, to, avatarUrl, name, last, onDismiss }) {
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          color: TEXT_MUTED, opacity: hov ? 1 : 0, transition: transition.hover,
        }}
      >
...
export default function Recommendations({ feed }) {
  ...
  const items = useMemo(() => {
    ...
    (feed.researchers || []).slice(0, 1).forEach(r => {
      out.push({
        id:        `collaborator-${r.id || r.full_name}`,
        type:      "Collaborator",
        icon:      User,
        title:     r.full_name,
    ...
    return out.filter(item => !dismissed.has(item.id));
  }, [feed, dismissed]);
