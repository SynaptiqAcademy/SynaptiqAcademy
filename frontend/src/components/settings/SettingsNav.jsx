...
import {
  ...
  Keyboard, Accessibility, FlaskConical, ShieldCheck, CloudCog,
} from "lucide-react";
import { Card } from "@/components/ds/Card";
import { SearchBar } from "@/components/ds/SearchBar";
import { List, ListItem } from "@/components/ds/List";
import { Label, Body, Caption, BodySmall } from "@/components/ds/Typography";
import { NAVY, BRD, TEXT_MUTED, TEXT_PRIMARY, WHITE } from "@/lib/tokens";
  ...
  return (
    <div className="w-full lg:w-[200px] lg:sticky lg:top-6" style={{ flexShrink: 0, alignSelf: "flex-start" }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <Body as="div" style={{ fontSize: 13, fontWeight: 600, marginTop: 2 }} color={TEXT_PRIMARY}>
      ...
      {results !== null ? (
        <List border={false} radius={0} style={{ background: "transparent" }}>
          {results.length === 0 ? (
            <Caption style={{ padding: "8px 4px" }}>No matches for "{query}"</Caption>
          ) : (
            results.map((r) => {
              ...
              return (
                <ListItem
                  ...
                  style={{ borderBottom: "none", padding: "7px 10px" }}
                >
          ...
          {CATEGORIES.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <ListItem
                ...
                selected={active}
                onClick={() => onSelect(id)}
                leading={<Icon size={14} style={{ flexShrink: 0, color: active ? WHITE : TEXT_MUTED }} />}
                style={{
                  borderBottom: "none",
                  borderRadius: 7,
      ...
      <Card padding="lg" style={{ marginTop: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
          ...
          <BodySmall as="div" style={{ fontWeight: 700 }} color={TEXT_PRIMARY}>Settings Sync</BodySmall>
        </div>
        <Caption as="p" style={{ lineHeight: 1.6, margin: 0 }}>
